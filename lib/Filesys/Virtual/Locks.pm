...
# If TEST_STORE is true, the module will use Data::Dumper to dump a plain
# text lock database. If it is false, then Storable will be used.
use constant TEST_STORE => 0;
...
sub new {
    ...
    if ( -e $file ) {
        ...
        return $this unless $@;
        print STDERR "ERROR LOADING LOCKS DB $file: $@";
...
sub _unlock {
    ...
    if (TEST_STORE) {
        ...
        print $h Data::Dumper->Dump( [ $this->{db} ] );
    }
    else {
...
sub removeLock {
    ...
    # Clean up all traces of the lock. There are two references to the
    # lock; the one in the top level hash, and the one in the tree. We
    # need to remove both. If one or other is missing, then there's
    # something wrong with the DB, but we'll try to recover anyway.
        ...
        if ( defined $lock ) {
            ...
            if ( $node && $node->{locks} ) {
                    ...
                    && !scalar( keys %{ $node->{subnodes} } ) )
                {
                    $node = pop(@parents);
                    if ($node) {
                        my $child = pop(@parents);
                        delete $node->{subnodes}->{$child};
                    }
                }
            }
        }
...
# $recurse can be 0 (only this node) 1 (this node and children) or -1
# (infinite) to inspect those resources.
sub getLocks {
...
# Get unexpired auth token, or undef if it does not exist (has expired)
sub getAuthToken {
    my ( $this, $token ) = @_;
...
# Remove the given auth token if it exists
sub removeAuthToken {
    my ( $this, $token ) = @_;
...
Author: (lock tokens) Sven Meyer, Modell Aachen
